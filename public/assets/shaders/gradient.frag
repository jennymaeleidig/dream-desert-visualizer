...
// adapted from: https://alexharri.com/blog/webgl-gradients

// 2d animated simplex noise
float bg_noise(){
  const float L = 0.0017; // noise scale (zoom)
  ...
  noise += snoise(vec3(gl_FragCoord.x * L - F * 0.4, gl_FragCoord.y * Y_SCALE * L * 0.70, u_time * S)) * 0.22;

  // result used as an alpha value so needs to be clamped approriately
  return clamp(noise, 0.0, 1.0);
}

void main(){
  // render out the raw texture that is provided
  // note: update the texture index based on the textures provided
  vec4 texColor = texture2D(u_base, vertTexCoord.xy);

  //ignore transparent pixels since input img is a png
  if (texColor.a > 0.0){
    float t = bg_noise(); //amt to mix between two colors

    vec3 color = mix(color_1, color_2, t);
    //apply computed pixel value
    texColor.rgb = color;
  }
  //output final color
